...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    //step1: 如果sequence_cnt != cur_kf->sequence，则新建一个新的图像序列
    ...
    //step2:获取当前帧的位姿vio_P_cur、vio_R_cur并更新
	...
	if (loop_index != -1)
	{
        ...
        //step4.1: 将当前帧与回环帧进行描述子匹配，如果成功则确定存在回环
        if (cur_kf->findConnection(old_kf))
        {
            ...
            //step4.2: 计算当前帧与回环帧的相对位姿, 纠正当前帧位姿w_P_cur、w_R_cur
            ...
            // step4.3: 如果存在多个图像序列，则将所有图像序列都合并到世界坐标系下
            ...
            {  
                ...
                vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
                vio_R_cur = w_r_vio *  vio_R_cur;
                cur_kf->updateVioPose(vio_P_cur, vio_R_cur);//根据偏移量shift_r shift_t跟新当前关键帧的vio位姿
                list<KeyFrame*>::iterator it = keyframelist.begin();
                for (; it != keyframelist.end(); it++)   //遍历keyframelist每一个keyframe
                {
                    if((*it)->sequence == cur_kf->sequence)//如果是在相同序列，通过偏移量shift_r shift_t更新他们vio位姿
                    {
                        Vector3d vio_P_cur;
                        Matrix3d vio_R_cur;
            ...
            // step4.4: 将当前帧放入优化队列中
            m_optimize_buf.lock();
            optimize_buf.push(cur_kf->index);//在另一个单独创建的线程中进行优化，（optimize_buf不为空就启动优化）
            m_optimize_buf.unlock();
        }
	}
    // step5: 获取VIO当前帧的位姿P、R，根据偏移量计算得到实际位姿。并进行位姿更新
	m_keyframelist.lock();
    Vector3d P;
    Matrix3d R;
    ...
    // step8: 绘制可视化轨迹中帧间的连线
    if (SHOW_S_EDGE)//sequential edge(前面4个关键帧之间连线)
    {
    ...
    if (SHOW_L_EDGE)//loop edge，loop帧之间连线
    {
        if (cur_kf->has_loop)
        {
            //printf("has loop \n");
    ...
    // step9: 发布topic：pub_pg_path、pub_path、pub_base_path
	keyframelist.push_back(cur_kf);
    publish();
	m_keyframelist.unlock();
}

//加载位姿图时候，loadKeyFrame时候不需要进行回环检测
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    //step1: 查询字典数据库，得到与每一帧的相似度评分ret
    QueryResults ret;
    TicToc t_query;
    db.query(keyframe->brief_descriptors, ret, 4, frame_index - 50); // 4个最相似返回，最近50帧不可能存在回环
    //printf("query time: %f", t_query.toc());
    //cout << "Searching for Image " << frame_index << ". " << ret << endl;



    // step2: 添加当前关键帧到字典数据库中
    TicToc t_add;
...
*/
    //step4: 系统开始的前50帧不进行回环, 如果在先前检测到回环候选帧再判断
    if (find_loop && frame_index > 50)
    {
        int min_index = -1;
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            if (min_index == -1 || (ret[i].Id < min_index && ret[i].Score > 0.015))//选择 score>0.015且距离最远的帧
                min_index = ret[i].Id;
        }
        return min_index; //返回最早的回环候选帧
    }
    else
        return -1;
    ...
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            ceres::LocalParameterization* angle_local_parameterization =
                AngleLocalParameterization::Create();//指定yaw角优化变量的迭代更新方式
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                if ((*it)->index < first_looped_index)
                    continue;//需要优化的关键帧为从第一次闭环帧到当前帧间的所有关键帧
                (*it)->local_index = i;
                Quaterniond tmp_q;
                Matrix3d tmp_r;
                Vector3d tmp_t;
                ...
                //add point
                problem.AddParameterBlock(euler_array[i], 1, angle_local_parameterization);//优化变量+维度  
                problem.AddParameterBlock(t_array[i], 3);

                if ((*it)->index == first_looped_index || (*it)->sequence == 0) // 固定优化的第一帧（闭环帧）
                {   
                    problem.SetParameterBlockConstant(euler_array[i]);
                    problem.SetParameterBlockConstant(t_array[i]);
                }
               /**
                 * 顺序边的测量方程：p̂_{ij}^{j} = {R̂_wb}^{-1} (p̂_i^w - p̂_j^w)
                 *                \hat{ψ}_ij = \hat{ψ}_i − \hat{ψ̂}_j
                 ...
                 * |帧1|    |帧2|    |帧3|     |帧4|     |帧5|
                */
                //add edge(sequentail edge)
                for (int j = 1; j < 5; j++) //和前面4个keyframe的边
                {
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j]) //i-j == 前1..4帧
                  {
                    Vector3d euler_conncected = Utility::R2ypr(q_array[i-j].toRotationMatrix());
                    Vector3d relative_t(t_array[i][0] - t_array[i-j][0], t_array[i][1] - t_array[i-j][1], t_array[i][2] - t_array[i-j][2]);
                    relative_t = q_array[i-j].inverse() * relative_t;//j系下
...
void PoseGraph::loadPoseGraph()
{
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        loadKeyFrame(keyframe, 0);//加载新构造的关键帧
        if (cnt % 20 == 0)
        {
            publish();
        }
