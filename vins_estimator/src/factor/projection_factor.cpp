...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;  // i时刻相机坐标系点
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; // 
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;  //世界系
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); //
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); //从i 投影到 j 时刻的 相机坐标点
    ...
    residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>(); // 投影-观测 
#endif
    ...
    if (jacobians)
    {
        ...
        Eigen::Matrix3d ric = qic.toRotationMatrix(); // 外参
        Eigen::Matrix<double, 2, 3> reduce(2, 3); //链式法则，这里先算残差对fcj的导数
#ifdef UNIT_SPHERE_ERROR
...
void ProjectionFactor::check(double **parameters)
{
    ...
    puts("num");
    std::cout << residual.transpose() << std::endl;  //得到residual
    ...
    std::cout << num_jacobian << std::endl;  //输出19次的计算差值
}
