...
class IntegrationBase
{
  public:
    ...
    void push_back(double dt, const Eigen::Vector3d &acc, const Eigen::Vector3d &gyr) //进行 积分
    {
        dt_buf.push_back(dt);
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;   //中值角速度
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2); //得到 q(旋转)的中值积分
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt;
        result_delta_v = delta_v + un_acc * _dt;
        result_linearized_ba = linearized_ba;   // = 上时刻的 ba 
        ...
        if(update_jacobian) 
        {
            ...
            //step_jacobian = F;
            //step_V = V;
            jacobian = F * jacobian; //迭代求 当前时刻预积分量对上一时刻的预积分量的jacobian, 最终得到该阶段的预积分量对初始时刻的状态量的jacobian, 其实是为了该阶段的预积分量对 初始时刻 ba bg 的导数
            covariance = F * covariance * F.transpose() + V * noise * V.transpose(); // 根据F和G矩阵计算预积分误差的协方差
        }
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        // IMU残差计算公式, 用两个时刻的状态量对两个时刻的预积分量进行约束
    ...
    double dt;
    Eigen::Vector3d acc_0, gyr_0;   //前一帧 陀螺仪 加速度计测量值
    Eigen::Vector3d acc_1, gyr_1;   //后一帧 陀螺仪 加速度计测量值

    const Eigen::Vector3d linearized_acc, linearized_gyr; //这一段预积分初始时刻的IMU测量值，作为常量一直保存，在IntegrationBase对象创建时指定
    Eigen::Vector3d linearized_ba, linearized_bg;    //这段预积分对应加速度偏置、陀螺仪偏置

    Eigen::Matrix<double, 15, 15> jacobian, covariance;// 当前时刻预积分量关于预积分初始时刻状态量的jacobian  ，预积分误差的协方差
    Eigen::Matrix<double, 18, 18> noise; // 传播过程中噪声的协方差

    double sum_dt;   //这段预积分总的时间间隔
    //该段预积分量, i->k (Pik, Qik, Vik)
