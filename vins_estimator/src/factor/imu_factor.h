...
//delta_q = Qi.inverse() * Qj;
        ...
        Eigen::Map<Eigen::Matrix<double, 15, 1>> residual(residuals); // residuals 传出参数
        ...
        {
            //得到 P Q V 对 i时刻的ba bg 导数(根据预积分时候迭代得到的)
...
///                ROS_BREAK();
            ...
            if (jacobians[0]) // 残差对 i 时刻的 pose(7维) 求导
            {
            ...
            if (jacobians[1]) // 残差对 i 时刻的 v ba bg(9维) 求导
            {
            ...
            if (jacobians[2]) // 残差对 j 时刻的 pose (7维) 求导
            {
            ...
            if (jacobians[3])// 残差对 j 时刻的 v ba bg(9维) 求导
            {
